...
public class Patrol : Physics2DObject
{
    ...
    void Start()
    {
        ...
        remoteTarget = GameObject.FindWithTag("Player").transform;
    }
    ...
    public void FixedUpdate()
    {
        ...
        if (followTarget)
        {
            rigidbody2D.MovePosition(transform.position  + ((Vector3)currentTarget - transform.position + new Vector3(remoteTarget.position.x, 0.0f)).normalized * speed * Time.fixedDeltaTime);

            if (Vector2.Distance(transform.position, ((Vector3)currentTarget + new Vector3(remoteTarget.position.x, 0.0f))) <= .1f)
            {
